  ...
  return (
    <div className="flex flex-col items-center md:pt-12.5">
      <div className="flex flex-col gap-4 md:gap-2 w-full items-center">
        ...
        <p className="text-black font-sans md:text-sm md:font-normal md:tracking-tighter text-sm xl:text-[20px] leading-tight text-center font-normal">
      ...
      <div className="md:mt-8 lg:mt-9 grid md:grid-cols-4 gap-3 w-full">
        {images.map((img: any, index) => (
          <Image
            alt={img.image}
            className="h-50 md:h-116 md:rounded-sm lg:h-125 xl:h-180 object-cover w-full rounded-xs"
            height={300}
            width={400}
            src={img.image}
            key={`image_${index}`}
      ...
      <div className="flex w-full justify-center items-center">
        <button className="md:w-[30%] md:text-[13px] md:font-thin md:mt-8 lg:mt-9 lg:p-4 md:p-3.5 w-full mt-4 font-sans font-normal text-white text-base bg-[#304B39] p-4 rounded-sm">Shop All</button>
      </div>
    </div>
  );
}
